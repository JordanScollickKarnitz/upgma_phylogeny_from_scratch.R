############################################################
# UPGMA Phylogenetic Tree Construction (From Scratch)
# Author: Jordan Scollick-Karnitz
# Language: R
#
# Description:
# Implements the UPGMA (Unweighted Pair Group Method
# with Arithmetic Mean) clustering algorithm using
# Jukes–Cantor distances to construct a phylogenetic tree.
############################################################
...
jukes_cantor_distance <- function(seq1, seq2) {
  ...
  # Proportion of mismatched sites
...
############################
# UPGMA clustering setup
############################

active_clusters <- seq_names
cluster_sizes <- rep(1, n)
names(cluster_sizes) <- seq_names
...
update_matrix_upgma <- function(mat, cluster1, cluster2, new_cluster) {
...
while (length(active_clusters) > 1) {
  ...
  cluster_sizes[new_node] <-
    cluster_sizes[c1] + cluster_sizes[c2]
  
  current_matrix <-
    update_matrix_upgma(current_matrix, c1, c2, new_node)
}
...
plot_upgma_tree <- function(tree_edges, tips) {
...
plot_upgma_tree(tree_edges, seq_names)
